...
class AlpacaProcessor:
    def __init__(self, API_KEY=None, API_SECRET=None, API_BASE_URL=None, api=None, data_type = "stock"):
    ...
    def _fetch_crypto_data_for_ticker(self, tickers, start_date, end_date, time_interval):
    ...
    def download_data(self, ticker_list, start_date, end_date, time_interval) -> pd.DataFrame:
        ...
        - data_type: string representing the data type ('stock', 'crypto')
        ...
        with ThreadPoolExecutor(max_workers=10) as executor:
            futures = []
            if self.data_type == "stock" or 'VIXY' in ticker_list:
                futures = [executor.submit(self._fetch_data_for_ticker, 
                                           ticker, 
                                           start_date, end_date, 
                                           time_interval
                                           ) for ticker in ticker_list]
                
            elif self.data_type == "crypto":
                futures = [executor.submit(self._fetch_crypto_data_for_ticker, 
                                           ticker_list, 
                                           start_date, end_date, 
                                           time_interval
                                           )]
    ...
    def clean_individual_ticker(args):
        ...
        tmp_df = tmp_df.merge(tic_df[["open", "high", "low", "close", "volume","trade_count","vwap"]], left_index=True, right_index=True, how='left')
        ...
        if pd.isna(tmp_df.iloc[0]["close"]):
            ...
            if first_valid_index is not None:
                ...
                tmp_df.iloc[0] = [first_valid_price] * 6 + [0.0]  # Set volume to zero
            else:
        ...
        for i in range(1, tmp_df.shape[0]):
            if pd.isna(tmp_df.iloc[i]["close"]):
                ...
                tmp_df.iloc[i] = [previous_close] * 6 + [0.0]
    ...
    def fetch_latest_data(
        ...
        for tic in ticker_list:
            tmp_df = pd.DataFrame(
                columns=["open", "high", "low", "close", "volume","trade_count","vwap"], index=times
            )
