...
class StockTradingEnvV2(gym.Env):
    ...
    def _sell(self, price, index, actions):
        ...
        if price[index] > 0:  # Sell only if current asset is > 0
            ...
            elif perc_diff < 0 and perc_diff > STOPLOSS_LIMIT:
                loss = diff_sell_buy_amount

            elif perc_diff <= STOPLOSS_LIMIT:
    ...
    def _buy(self, price, index, actions):
        if (price[index] > 0):  # Buy only if the price is > 0 (no missing data in this particular date)
            ...
            self.stocks_avg_buy_price[index] = ((self.stocks[index] * self.stocks_avg_buy_price[index])
                                                + (buy_num_shares + price[index])) / (self.stocks[index] + buy_num_shares)
